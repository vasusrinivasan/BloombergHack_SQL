...
def unique_phone_validator(form, field):
    # Convert string to list of characters
    phonenumber = list(field.data)
    phonenumber_length = len(phonenumber)
    ...
    is_valid = phonenumber_length>=10 and digits==phonenumber_length
    if not is_valid:
        raise ValidationError(_('Phone numbers must be at least 10 digits. Digits only please :('))
...
class RegisterForm(Form):
    ...
    email = StringField(_('Email'), validators=[
        ...
        validators.Email(_('Invalid Email')),
        unique_email_validator])
    phonenumber = StringField(_('Phone Number'), validators=[
        validators.Required(_('A Phone Number is required')),
